...
export default function Wordiff({initialMode='lexical',initialLevel='medium'}:{initialMode?:Mode;initialLevel?:Level}){
 ...
 return <main className="workspace">
  <header className="topbar"><Link className="brand" href="/"><span className="brandmark"><ScanText size={21}/></span>版本比对器<span className="beta">LAB</span></Link><span className="top-caption">原文与 AI 改写逐处比对</span><Link href="/canvas" className="download-button">Agent 画布 ↗</Link><AuthButton/><Button variant="ghost" onClick={()=>setMethods(true)}><BookOpen/> 算法原理 <ArrowUpRight size={14}/></Button><a className="download-button" href="/data/all-versions.md" download><Download size={14}/><span>下载四篇文章</span></a></header>
  ...
  <footer className="workspace-footer"><Link href="/privacy">隐私说明</Link><span><i className="tiny-dot"/>全文比对 · 本地预计算</span><span>颜色表示相似程度，不能判定作者归属或事实真伪，也不能断定原意未变。</span><Button size="xs" variant="ghost" onClick={()=>setMethods(true)}>算法原理与编辑说明 <ArrowUpRight size={12}/></Button></footer>
