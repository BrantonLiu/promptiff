...
const labels: Record<EngagementMode, string> = {
  review: '审阅模式', diff: '逐句 Diff', lexical: '分词热力图', semantic: '语义热力图',
};
  ...
  return <>
    <Dialog open={open} onOpenChange={next => { if (!next && open && !submitting) finish('dismissed'); }}>
      <DialogContent className="feedback-dialog sm:max-w-lg" showCloseButton={!submitting}>
        <DialogHeader>
          <DialogTitle>这四种比对方式，哪种帮到了你？</DialogTitle>
          <DialogDescription>你已体验全部视图。想听听你的感受，约半分钟，可匿名提交。</DialogDescription>
        </DialogHeader>
        <form onSubmit={submit} className="grid gap-5">
          <fieldset className="grid gap-2" disabled={submitting}>
            <legend className="mb-2 font-medium">1. 它对你辨别改写有多大帮助？</legend>
          ...
          <label className="grid gap-2 font-medium" htmlFor="feedback-preferred-mode">2. 哪种视图最有帮助？
            <select id="feedback-preferred-mode" value={preferredMode} onChange={event => setPreferredMode(event.target.value as EngagementMode)} required disabled={submitting} className="h-10 w-full rounded-lg border border-border bg-background px-3 text-sm font-normal">
              <option value="" disabled>请选择一种视图</option>
          ...
          <p className="text-xs text-muted-foreground">提交后将保存你的回答及本次体验的视图和前台停留时长。请勿填写个人敏感信息。</p>
    ...
    {thanks && <output className="fixed bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-3 rounded-xl border border-border bg-background px-4 py-3 text-sm shadow-lg">谢谢，你的反馈已收到。<Button variant="ghost" size="sm" onClick={() => setThanks(false)}>关闭</Button></output>}
  </>;
}
